...
class YearlyDataController extends Controller
{
    public function fetchYearlyData()
    {
        // Connect to the MongoDB database
        $mongoDB = DB::connection('mongodb');

        // Set the selected date to the first day of the current year
        $selectedDate = date('2023-01-01');
        ...
        if (!is_null($meterTag)) {
            ...
            $startDate = date('Y-01-01', strtotime($selectedDate));
            $endDate = date('Y-12-31', strtotime($selectedDate));

            // Build the query with the specific meter tag and date range for the current year
            $query = [
